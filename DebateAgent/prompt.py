...
<SEARCH_RESULTS> # Retrieved data or evidence relevant to the query
<HYPOTHESIS>     # The candidate hypothesis generated by a downstream agent

Using these three inputs, generate a **single concise debate topic** that both proponent and opponent agents can argue about.

------------------------------------
### Objective
Transform the scientific hypothesis into a **debate topic**, by expressing:

1. The **central scientific issue or claim** implied by the hypothesis.  
2. The **key comparison or condition** that frames the debate  
   (e.g., catalyst structure differences, active site configurations, support choice).  
3. The **measurable outcome or performance dimension** being contested  
   (e.g., activity, selectivity, stability, efficiency).

------------------------------------
### Instructions
- Output must be **a debate topic, not the hypothesis itself**.
- Keep it **short (one sentence is ideal; maximum two)**.
- Phrase it as a **neutral, discussable scientific question or proposition**,  
  not as a factual statement to be tested.
- The topic should be **broad enough for argument**, yet **specific enough** to stay grounded in the hypothesis.
- Do not include experimental details.
- Do not summarize inputs; **convert them into a debate-friendly topic**.
